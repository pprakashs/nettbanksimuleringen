...
const CreateAutoPayment = ({ accounts, autoPayments }) => {
	const [tooltip, setTooltip] = useState(false);
	const [selectedAccount, setSelectedAccount] = useState({});
	const [selectedAutoPayment, setSelectedAutoPayment] = useState({});
	...
	useEffect(() => {
		...
		if (autoPayments.selectedAutoPaymentSuggestion !== null) {
			setSelectedAutoPayment(...autoPayments.autoPaymentSuggestion.filter((acc) => acc.accountNumber === autoPayments.selectedAutoPaymentSuggestion));
		}
	}, []);
	...
	console.log(autoPayments.selectedAutoPaymentSuggestion);
	const submitHandle = (e) => {
		...
		const data = {
			amount: amount * 1,
			name: selectedAutoPayment.name,
			accountNumber: autoPayments.selectedAutoPaymentSuggestion,
		};
										...
										<div className="relative">
											...
											>
												...
												<div className="ml-auto">
													Disponibel saldo: <strong className="font-anenirHeavy">{numberFormat(selectedAccount.remainingAmount)}</strong>
												</div>
											</button>
											<svg
												...
												.map((account, index) => (
													<li key={index}>
														...
														>
															...
															<div className="ml-auto">
																Disponibel saldo: <strong className="font-anenirHeavy">{numberFormat(account.remainingAmount)}</strong>
															</div>
														</button>
													</li>
												))}
